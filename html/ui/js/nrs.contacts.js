...
var NRS = (function(NRS, $) {
			...
			if (contacts && contacts.length) {
				...
				$.each(contacts, function (index, contact) {
					...
					rows += "<tr>" +
						...
						"<a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_money_modal' data-contact='" + String(contact.name).escapeHTML() + "'>" + $.t("send_wng") + "</a>&nbsp;" +
	...
	NRS.forms.addContact = function($modal) {
		...
		if (/^\d+$/.test(data.name) || /^WNG\-/i.test(data.name)) {
			return {
		...
		var address;
		if (/^WNG\-/i.test(data.account_id)) {
			data.account_rs = data.account_id;
			address = new WngAddress();
	...
	$("#update_contact_modal").on("show.bs.modal", function(e) {
		...
		if (!contactId && NRS.selectedContext) {
			...
			var dbKey = (/^WNG\-/i.test(accountId) ? "accountRS" : "account");
			var dbQuery = {};
