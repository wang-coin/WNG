...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, isAsync) {
        ...
        try {
            var wngFields = [
                ...
                ["priceWNG", "priceNQT"],
                ["refundWNG", "refundNQT"],
                ["discountWNG", "discountNQT"],
                ["phasingQuorumWNG", "phasingQuorum"],
                ["phasingMinBalanceWNG", "phasingMinBalance"],
                ["controlQuorumWNG", "controlQuorum"],
                ["controlMinBalanceWNG", "controlMinBalance"],
                ["controlMaxFeesWNG", "controlMaxFees"],
                ["minBalanceWNG", "minBalance"],
                ["shufflingAmountWNG", "amount"],
                ["monitorAmountWNG", "amount"],
                ["monitorThresholdWNG", "threshold"]
            ];

            for (i = 0; i < wngFields.length; i++) {
                var wngField = wngFields[i][0];
                var nqtField = wngFields[i][1];
                if (wngField in data) {
                    data[nqtField] = NRS.convertToNQT(data[wngField]);
                    delete data[wngField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToWNG(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var wngAddress = new WngAddress();
            var accountRS = "";
            if (wngAddress.set(accountId)) {
                accountRS = wngAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, isAsync) {
        ...
        var url = NRS.server + "/wng?requestType=" + requestType;
    ...
    NRS.broadcastTransactionBytes = function (transactionData, callback, originalResponse, originalData) {
        $.ajax({
            url: NRS.server + "/wng?requestType=broadcastTransaction",
            crossDomain: true,
            dataType: "json",
            type: "POST",
            timeout: 30000,
