...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedWngAccounts") && NRS.getStrItem("savedWngAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedWngAccounts").split(";");
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedWngAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedWngAccounts');
		} else {
			NRS.setStrItem("savedWngAccounts", accounts);
		}
		NRS.listAccounts();
	};
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					if ($("#remember_account").is(":checked") || NRS.newlyCreatedAccount) {
						var accountExists = 0;
						if (NRS.getStrItem("savedWngAccounts")) {
							accounts = NRS.getStrItem("savedWngAccounts").split(";");
							$.each(accounts, function(index, account) {
						...
						if (!accountExists){
							if (NRS.getStrItem("savedWngAccounts") && NRS.getStrItem("savedWngAccounts") != ""){
								accounts = NRS.getStrItem("savedWngAccounts") + NRS.accountRS + ";";
								NRS.setStrItem("savedWngAccounts", accounts);
							} else {
								NRS.setStrItem("savedWngAccounts", NRS.accountRS + ";");
							}
						}
					}
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Wng passphrase.";
			html += "</p>";
			...
			html += "you can loose your WNG! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of WNG now or in the future.";
			html += "</p>";
			html += "</div>";
