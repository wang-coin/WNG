...
var NRS = (function(NRS, $) {
			...
			html += "<table width='100%' style='table-layout:fixed;'>";
				html += "<tr>";
					...
					html += "<td>";
						...
						html += "<div class='price'><strong>" + NRS.formatAmount(good.priceNQT) + " WNG</strong></div>";
	...
	NRS.getMarketplacePurchaseHTML = function(purchase, showBuyer) {
		...
		return "<div data-purchase='" + String(purchase.purchase).escapeHTML() + "' " + (purchase.unconfirmed ? "class='tentative'" : "") + "><div style='float:right;color: #999999;background:white;padding:5px;border:1px solid #ccc;border-radius:3px'>" +
			...
			"<tr><td><strong>" + $.t("price") + "</strong>:</td><td>" + NRS.formatAmount(purchase.priceNQT) + " WNG</td></tr>" +
	...
	NRS.dgs_search_main = function(callback) {
		...
		$(".dgs_search input[name=q]").val("").trigger("unmask").mask("WNG-****-****-****-*****");
         ...
         var rows = "";
			if (response.goods && response.goods.length) {
				...
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<td class='price numeric'>" + NRS.formatAmount(good.priceNQT, false, false, priceDecimals) + " WNG</td>";
	...
	NRS.forms.dgsPriceChangeComplete = function(response, data) {
		...
		$("#my_dgs_listings_table").find("tr[data-goods=" + String(data.goods).escapeHTML() + "]").addClass("tentative").find(".price").html(NRS.formatAmount(data.priceNQT) + " WNG");
	};
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGood", {
					...
					if (good.errorCode) {
						...
						output += "<tr><th><strong>" + $.t("price") + "</strong>:</th><td>" + NRS.formatAmount(response.priceNQT) + " WNG</td></tr>";
						...
						if (type == "dgs_refund_modal" || type == "dgs_delivery_modal" || type == "dgs_feedback_modal") {
							...
							if (response.quantity != "1") {
								...
								output += "<tr><th><strong>" + $.t("total") + "</strong>:</th><td>" + orderTotal + " WNG</td></tr>";
							}
							if (response.discountNQT && (type == "dgs_refund_modal" || type == "dgs_feedback_modal")) {
								output += "<tr><th><strong>" + $.t("discount") + "</strong>:</th><td>" + NRS.formatAmount(response.discountNQT) + " WNG</td></tr>";
							}
						}
						...
						if (type == "dgs_view_refund_modal") {
							output += "<tr><th><strong>" + $.t("refund_price") + "</strong>:</th><td>" + NRS.formatAmount(response.refundNQT) + " WNG</td></tr>";
						}
						...
						if (type == "dgs_refund_modal") {
							...
							$("#dgs_refund_refund").val(NRS.convertToWNG(refund));
	...
	$("#dgs_product_modal, #dgs_delisting_modal, #dgs_quantity_change_modal, #dgs_price_change_modal, #dgs_purchase_modal").on("show.bs.modal", function(e) {
		...
		NRS.sendRequest("getDGSGood", {
			...
			if (type == "dgs_quantity_change_modal") {
				...
				$("#dgs_price_change_current_price, #dgs_price_change_price").val(NRS.convertToWNG(response.priceNQT).escapeHTML());
			} else if (type == "dgs_purchase_modal") {
				$modal.find("input[name=recipient]").val(response.sellerRS);

				$("#dgs_purchase_price").val(String(response.priceNQT).escapeHTML());
				$("#dgs_total_purchase_price").html(NRS.formatAmount(response.priceNQT) + " WNG");
				...
				$("#dgs_purchase_quantity").on("change", function() {
					...
					$("#dgs_total_purchase_price").html(NRS.formatAmount(totalNQT) + " WNG");
				});
			}
			}
		},false);
    ...
    dgsShowPictureModal.on("show.bs.modal", function(e) {
		...
		NRS.sendRequest("getDGSGood+", {
			...
			if (!response.hasImage) {
				...
				image = "/wng?requestType=downloadPrunableMessage&transaction=" + response.goods + "&retrieve=true";
			}
    ...
    NRS.dgs_get_picture = function(input) {
        ...
        if (!input.hasImage) {
            ...
            picture.src = "/wng?requestType=downloadPrunableMessage&transaction=" + input.goods + "&retrieve=true";
