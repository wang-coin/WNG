...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumWNG",
			addonText: "WNG",
			addonI18n: "wng_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_wng', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceWNG';
		context['addonText'] = 'WNG';
		context['addonI18n'] = 'wng_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_wng', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
