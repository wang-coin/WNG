...
var NRS = (function(NRS, $) {
    ...
    NRS.getTaggedDataLink = function(transaction, isText) {
        if (isText) {
			...
			return "<a href='/wng?requestType=downloadTaggedData&transaction=" + String(transaction).escapeHTML() +
	...
	NRS.tagged_data_search_main = function(callback) {
		...
		$(".tagged_data_search input[name=q]").val("").trigger("unmask").mask("WNG-****-****-****-*****");
    ...
    $("#tagged_data_view_modal").on("show.bs.modal", function(e) {
		...
		$("#tagged_data_download").attr("href", "/wng?requestType=downloadTaggedData&transaction=" + transaction + "&retrieve=true");
    });
    ...
    $("#extend_data_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getTransaction", {
            ...
            var fee = NRS.convertToWNG(String(response.feeNQT).escapeHTML());
            $('#extend_data_fee').val(fee);
            $('#extend_data_fee_label').html(String(fee) + " WNG");
        })
    });
