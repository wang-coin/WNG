...
import wng.crypto.AnonymouslyEncryptedData;
...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Wng.getBooleanProperty("wng.deleteFinishedShufflings");
    ...
    private void cancel(Block block) {
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f WNG awarded to forger at height %d", ((double)fee) / Constants.ONE_WNG, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f WNG awarded to forger at height %d", ((double)fee) / Constants.ONE_WNG, block.getHeight());
        }
