...
import static wng.user.JSONResponses.NOTIFY_OF_ACCEPTED_TRANSACTION;
...
public final class SendMoney extends UserServlet.UserRequestHandler {
    ...
    @Override
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws WngException.ValidationException, IOException {
        if (user.getSecretPhrase() == null) {
            return null;
        }

        String recipientValue = req.getParameter("recipient");
        String amountValue = req.getParameter("amountWNG");
        String feeValue = req.getParameter("feeWNG");
        ...
        try {
            ...
            amountNQT = Convert.parseWNG(amountValue.trim());
            feeNQT = Convert.parseWNG(feeValue.trim());
            ...
            response.put("amountWNG", amountValue);
            response.put("feeWNG", feeValue);
        ...
        } else if (feeNQT < Constants.ONE_WNG || feeNQT > Constants.MAX_BALANCE_NQT) {

            JSONObject response = new JSONObject();
            response.put("response", "notifyOfIncorrectTransaction");
            response.put("message", "\"Fee\" must be at least 1 WNG!");
            response.put("recipient", recipientValue);
            response.put("amountWNG", amountValue);
            response.put("feeWNG", feeValue);
            response.put("deadline", deadlineValue);
        ...
        if (account == null || Math.addExact(amountNQT, feeNQT) > account.getUnconfirmedBalanceNQT()) {
            ...
            final Transaction transaction = Wng.newTransactionBuilder(user.getPublicKey(),
