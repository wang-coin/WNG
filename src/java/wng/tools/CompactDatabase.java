 ...
 *
 *   java -cp "classes:lib/*:conf" wng.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dwng.runtime.mode=desktop wng.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Wng properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "wng.testDb" : "wng.db";
        String dbType = Wng.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Wng.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Wng.getDbDir(Wng.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Wng.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Wng.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Wng.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "wng.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "wng.mv.db");
            if (!dbFile.exists()) {
