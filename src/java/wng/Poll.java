...
public final class Poll extends AbstractPoll {
    ...
    private static final boolean isPollsProcessing = Wng.getBooleanProperty("wng.processPolls");
    ...
    public static DbIterator<Poll> getActivePolls(int from, int to) {
        return pollTable.getManyBy(new DbClause.IntClause("finish_height", DbClause.Op.GT, Wng.getBlockchain().getHeight()), from, to);
    }
    ...
    public static DbIterator<Poll> getPollsByAccount(long accountId, boolean includeFinished, boolean finishedOnly, int from, int to) {
        ...
        if (finishedOnly) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.LTE, Wng.getBlockchain().getHeight()));
        } else if (!includeFinished) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.GT, Wng.getBlockchain().getHeight()));
        }
    ...
    public static DbIterator<Poll> searchPolls(String query, boolean includeFinished, int from, int to) {
        DbClause dbClause = includeFinished ? DbClause.EMPTY_CLAUSE : new DbClause.IntClause("finish_height", DbClause.Op.GT, Wng.getBlockchain().getHeight());
    ...
    private Poll(Transaction transaction, Attachment.MessagingPollCreation attachment) {
        ...
        this.timestamp = Wng.getBlockchain().getLastBlockTimestamp();
    }
    ...
    public boolean isFinished() {
        return finishHeight <= Wng.getBlockchain().getHeight();
    }

    private List<OptionResult> countResults(VoteWeighting voteWeighting) {
        int countHeight = Math.min(finishHeight, Wng.getBlockchain().getHeight());
        if (countHeight < Wng.getBlockchainProcessor().getMinRollbackHeight()) {
            return null;
        }
