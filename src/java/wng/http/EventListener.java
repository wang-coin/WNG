...
import wng.TransactionProcessor;
 ...
 * has not been received within wng.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by wng.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Wng.getIntProperty("wng.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Wng.getIntProperty("wng.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Wng.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Wng.getTransactionProcessor();
    ...
    /** Wng event listeners */
    private final List<WngEventListener> wngEventListeners = new ArrayList<>();
     ...
     *
     * Wng event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Wng event registrations
     ...
     *
     * @param   eventRegistrations      Wng event registrations
     * @throws  EventListenerException  Invalid Wng event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<WngEventListener> it = wngEventListeners.iterator();
                while (it.hasNext()) {
                    WngEventListener listener = it.next();
                ...
                if (addListener) {
                    WngEventListener listener = new WngEventListener(event);
                    listener.addListener();
                    wngEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            wngEventListeners.forEach(WngEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Wng event listener
     */
    private class WngEventListener {

        /** Event handler */
        private final WngEventHandler eventHandler;

        /**
         * Create the Wng event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public WngEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Wng listener for this event
         */
        ...
        /**
         * Remove the Wng listener for this event
         */
        ...
        /**
         * Return the hash code for this Wng event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Wng events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof WngEventListener) &&
                    eventHandler.equals(((WngEventListener)obj).eventHandler));
        }

        /**
         * Wng listener event handler
         */
        private abstract class WngEventHandler {
            ...
            /**
             * Create the Wng event handler
             *
             * @param   eventRegistration   Event registration
             */
            public WngEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof WngEventHandler) &&
                                        owner == ((WngEventHandler)obj).owner &&
                                        accountId == ((WngEventHandler)obj).accountId &&
                                        event == ((WngEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends WngEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends WngEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends WngEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends WngEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Wng listener event
        ...
        /**
         * Return the Wng listener event
         *
         * @return                  Wng listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
