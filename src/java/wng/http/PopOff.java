...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Wng.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Wng.getBlockchainProcessor().popOffTo(Wng.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Wng.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Wng.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Wng.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
