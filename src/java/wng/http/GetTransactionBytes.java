...
import static wng.http.JSONResponses.INCORRECT_TRANSACTION;
import static wng.http.JSONResponses.MISSING_TRANSACTION;
...
public final class GetTransactionBytes extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        transaction = Wng.getBlockchain().getTransaction(transactionId);
        JSONObject response = new JSONObject();
        if (transaction == null) {
            transaction = Wng.getTransactionProcessor().getUnconfirmedTransaction(transactionId);
            if (transaction == null) {
                return UNKNOWN_TRANSACTION;
            }
        } else {
            response.put("confirmations", Wng.getBlockchain().getHeight() - transaction.getHeight());
        }
