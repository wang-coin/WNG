...
import wng.AccountRestrictions;
import wng.Alias;
import wng.Appendix;
import wng.Asset;
import wng.AssetDelete;
import wng.AssetTransfer;
import wng.Attachment;
import wng.Block;
import wng.Constants;
import wng.Currency;
import wng.CurrencyExchangeOffer;
import wng.CurrencyFounder;
import wng.CurrencyTransfer;
import wng.CurrencyType;
import wng.DigitalGoodsStore;
import wng.Exchange;
import wng.ExchangeRequest;
import wng.FundingMonitor;
import wng.Generator;
import wng.HoldingType;
import wng.MonetarySystem;
import wng.Wng;
import wng.Order;
import wng.PhasingPoll;
import wng.PhasingVote;
import wng.Poll;
import wng.PrunableMessage;
import wng.Shuffler;
import wng.Shuffling;
import wng.ShufflingParticipant;
import wng.TaggedData;
import wng.Token;
import wng.Trade;
import wng.Transaction;
import wng.Vote;
import wng.VoteWeighting;
import wng.crypto.Crypto;
import wng.crypto.EncryptedData;
import wng.db.DbIterator;
import wng.peer.Hallmark;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Wng.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWNG", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWNG", account.getEffectiveBalanceWNG(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWNG", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_WNG));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.WNG) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Wng.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Wng.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Wng.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Wng.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Wng.getBlockchain().getTransaction(entry.getEventId());
