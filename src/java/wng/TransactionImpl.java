...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws WngException.NotValidException {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = Wng.getEpochTime();
            }
            if (!ecBlockSet) {
        ...
        @Override
        public TransactionImpl build() throws WngException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws WngException.NotValidException {
        ...
        if (builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Wng.getBlockchain().getHeight());
        ...
        if (builder.signature != null && secretPhrase != null) {
            throw new WngException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new WngException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
        ...
        if (bytes == null) {
            try {
                ...
                if (useNQT()) {
                    ...
                    buffer.putInt((int) (amountNQT / Constants.ONE_WNG));
                    buffer.putInt((int) (feeNQT / Constants.ONE_WNG));
                    if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws WngException.NotValidException {
        try {
            ...
            if (buffer.hasRemaining()) {
                throw new WngException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (WngException.NotValidException|RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws WngException.NotValidException {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws WngException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new WngException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws WngException.NotValidException {
        try {
            ...
            if (transactionType == null) {
                throw new WngException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || Wng.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
    ...
    @Override
    public void validate() throws WngException.ValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeNQT != 0) : (deadline < 1 || feeNQT <= 0)
                || feeNQT > Constants.MAX_BALANCE_NQT
                || amountNQT < 0
                || amountNQT > Constants.MAX_BALANCE_NQT
                || type == null) {
            throw new WngException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
        ...
        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new WngException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new WngException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new WngException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new WngException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (! appendage.verifyVersion(this.version)) {
                throw new WngException.NotValidException("Invalid attachment version " + appendage.getVersion()
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new WngException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }

        if (!validatingAtFinish) {
            long minimumFeeNQT = getMinimumFeeNQT(Wng.getBlockchain().getHeight());
            if (feeNQT < minimumFeeNQT) {
                throw new WngException.NotCurrentlyValidException(String.format("Transaction fee %f WNG less than minimum fee %f WNG at height %d",
                        ((double) feeNQT) / Constants.ONE_WNG, ((double) minimumFeeNQT) / Constants.ONE_WNG, Wng.getBlockchain().getHeight()));
            }
        }
    ...
    private long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_WNG);
        }
        return totalFee;
    }
